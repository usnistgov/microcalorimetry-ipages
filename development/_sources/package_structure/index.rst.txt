...
The high level API is the primary interface of function which interact
with user interfaces (UX) - i.e the GUI or the CLI. The high level api is also available for use via
importing the package and envoking from a python script.

In general, the job of functions in the high level is to configuration objects
and basic python data types suitable for a user interface (strings, numers,
booleans). High level functions that perform some set of operations based on
...
and classes. These functions and classes are handed off to in the algorithms
controlled by the higherlevel API, and can't be directly interacted with
by a UX. However, they are available for use in a Python script by import.

Configuration objects act like adapters between the high-level API and the low
level API. High level functions take in configuration objects, which the high
level functions interpret and pass off to lower level API.In the case of measurement running functions, the lower level
...
in according to the appropriate algorithm.

.. raw:: html
    :file: processing_flow.drawio.svg

The goal of this architecture is to make facilitate adding new analysis functions
that operate will with the UX and via Python scripts, as well as make the code
modular. You can interact with the high level API for routine tasks, and
